...
**本项目唯一构建方式**：仅使用 `build_tools/final_build.sh` 脚本进行编译。

其他所有构建脚本（如 `xcode_build.sh`、`simple_build.sh`、`run_in_xcode.sh` 等）均已废弃并删除，请勿再使用或引用。
...
### 唯一方案：使用 final_build.sh（推荐）⭐

这是本项目**唯一**的构建方式，适合日常开发和版本发布。

> **注意**：其他构建脚本（如 `xcode_build.sh`、`simple_build.sh` 等）均已删除，请勿尝试使用。
...
| `&&`                                               | 前一个命令成功后执行下一个                     |
| `bash build_tools/final_build.sh`                  | **唯一构建脚本** - 执行编译                    |
| `2>&1`                                             | 将错误输出重定向到标准输出（便于查看完整日志） |

> **重要**：不要尝试使用其他已删除的脚本（如 `xcode_build.sh`、`simple_build.sh` 等）。
...
> **重要**：如果问题依然存在，请检查是否使用了正确的构建脚本（仅使用 `final_build.sh`）。
...
> **警告**：不要尝试使用其他已删除的构建脚本，这可能导致编译失败或产物异常。
...
> **重要提醒**：整个发布流程仅使用 `final_build.sh` 进行构建，不要使用其他任何构建脚本。
...
> **再次强调**：不要使用任何已删除的其他构建脚本（如 `xcode_build.sh`、`simple_build.sh`、`run_in_xcode.sh` 等）。

### 批量打包多个版本

```bash
# 为不同版本打包（仅使用 final_build.sh）
...
- macOS 15.0+
- Swift 5.10+
- 无需 Xcode（`final_build.sh` 使用 swiftc 直接编译）
