...
### v2.4.20 (当前开发版本) ✨ **最新**
- 🎯 **未完成任务转入策略优化**:
  - 任务转入今日后，原任务自动标记为"已放弃"，避免在全部任务中重复显示
  - 记录每个任务的累计转入次数，达到上限后不再自动转入
  - 全部任务视图优化："全部"筛选器只显示有效任务，"已放弃"可查看转入历史
  - 详细的转入日志输出，便于调试和验证
- 📝 **修改文件**: TaskStore+Tasks.swift (+6 行), AllTasksView.swift (+6 行)
- 📄 **新增文档**: V2.4.20_CARRY_OVER_STRATEGY_OPTIMIZATION.md
