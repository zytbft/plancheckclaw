...
- **一键添加到今日**：每个任务右侧显示"小太阳"图标（`sun.max.fill`），点击即可复制一份到今日列表
  - 图标位置：任务行最右侧，五角星图标之后
  - 悬停提示："添加到今日任务"
...
### v2.6.9 (当前开发版本) ✨ **最新**
- 🎯 **批量操作功能增强**: 在两个视图的批量菜单中新增三个实用功能
  - ✅ **全选功能**: 一键选中当前筛选条件下的所有任务
  - ✅ **添加到今日任务**: 在全部任务视图中批量复制任务到今日列表
  - ✅ **从今日任务移除**: 在今日任务视图中批量移除任务从"我的一天"
- 🎨 **右键菜单同步增强**: 两个视图的右键菜单也添加了相同功能
- 🔧 **智能过滤逻辑**: 自动过滤已加入的任务，避免重复操作
- 📝 **修改文件**: TodayView.swift (+44 行), AllTasksView.swift (+44 行)
- 📄 **实现报告**: V2.6.9_BATCH_OPERATIONS_ENHANCEMENT.md
